...
*                   Mike F (badgered Joe W into adding Joystick Code, and rewrote from scratch, to better understand how it all works, adding a keyboard/joystick switch, and a second function [SHIFT] button)
...
// Key press array, for each user. Up, Right, Down, Left, B1-B9
unsigned char keylist[4][13] =
{
	{ 0xE8, 0xE6, 0xE2, 0xE4, KEY_LEFT_CTRL, KEY_LEFT_ALT, 0x20, KEY_LEFT_SHIFT, 'z', 'x', 'c', '3', '1'},  // Player 1 Keyboard Keys
	{ 'r', 'g', 'f', 'd', 'a', 's', 'q', 'w', 'e', '[', ']', '4', '2'},									    // Player 2 Keyboard Keys
	{ 0xDF, 0xE6, 0xDE, 0xE4, KEY_F2, KEY_F7, KEY_F4, KEY_F8, KEY_F6, 'p', KEY_F1, '3', '1'},	            // Player 1 [SHIFT] keys
	{ 'r', 'g', 'f', 'd', 'a', 's', 'q', 'w', 'e', '[', ']', '4', '2'}                                      // Player 2 [SHIFT] keys
};
...
// B5		Z
// B6		X
// B7		C				Used for Select key in RetroArch
// B8		3				Player 1 Coin (COIN1) / Left Flipper
// B9		1				Player 1 Start
// B10      4               Player 2 Coin (COIN2) / Right Flipper   <-- Not set / used by default. Set to 4 to use. Edit Keylist also.
...
// B9		2				Player 2 Start
// B10      3               Plsyer 1 Coin (COIN1)                   <-- Not set / used by default.
...
void setup()
{
	...
	for (int index = 0; index < JOYSTICK_COUNT; index++)
	{
		...
		// Sets the last button state (unpressed) for the 4 directions
		...
		// Sets the last button state (unpressed) for all the buttons
	...
	Keyboard.releaseAll();  // Not required, but just in case.
							// Start the mouse
	Mouse.begin();          // Required for spinner. (Also, disable mouse acceleration in windows)
}
...
void loop()
{
	...
	else
	{
		...
		{
			//int v = digitalRead(dataInPin); // Read Pin MISO with digitalRead
			//int v = (PINB & 0b0000100) >> 2; // Read MISO Pin directly

			//int v = digitalRead(dataInPin); // Read Pin D17 (SS) with digitalRead
			int v = (PINB & 0b00000001) >> 0; // Read SS Pin directly
	...
	switch (currentKeyboardModeState)       // Swap case values to change default behaviour. Default is KEYB button is off, keyboard mode is off, joystick mode is on.
	{
		...
		case 1: // Key is not pressed, so enable JOYSTICK mode
			...
			for (int i = 0; i < JOYSTICK_COUNT; i = i + 1)
			{
				...
				//Iterate through the 9 buttons (0-8) assigning the current state of the pin for each button, HIGH(0b00000001) or LOW(0b00000000), to the currentState variable
				button[i] = 0;
